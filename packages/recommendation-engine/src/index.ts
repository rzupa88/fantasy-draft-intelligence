...
export function getReplacementLevels(state: DraftState): ReplacementLevel[] {
  ...
  return positions.map((position) => {
    const replacementRank = calculateReplacementRank(
      position,
...
function scoreCandidate(player: PlayerDataRecord, context: ScoringContext): CandidateScore {
  ...
  const expectedAvailability = calculateAvailabilityUrgency(
    player.adp,
  ...
  const picksUntilNextUserPick =
    context.nextUserPick === null ? null : context.nextUserPick - context.currentOverallPick;
...
function calculateBaseValue(player: PlayerDataRecord, context: ScoringContext): number {
  ...
  if (player.projected_points !== null) {
    weighted.push({
      value: normalizeHigher(player.projected_points, context.projectionValues),
      weight: 0.7,
    });
  }
  if (player.overall_rank !== null) {
    weighted.push({
      value: normalizeLower(player.overall_rank, context.overallRankValues),
      weight: 0.3,
    });
  }
...
function calculateTierUrgency(
  ...
  if (player.tier === null) {
    return { score: 35, sameTierRemaining: null };
  }
  ...
  const nextTierPlayer = samePosition.find(
    (candidate) => candidate.tier !== null && candidate.tier > player.tier!,
  );

  if (
    player.projected_points !== null &&
    nextTierPlayer !== undefined &&
    nextTierPlayer.projected_points !== null &&
...
function calculateRosterNeed(
  ...
  state.settings.rosterSlots.forEach((rule) => {
    for (let slotIndex = 1; slotIndex <= rule.count; slotIndex += 1) {
      if (filledSlots.has(`${rule.slot}:${slotIndex}`)) {
        continue;
      }
      if (rule.slot === "BENCH") {
        if (rule.eligiblePositions.includes(position)) {
          benchOpen += 1;
        }
      } else if (rule.eligiblePositions.includes(position)) {
        if (rule.eligiblePositions.length === 1) {
          dedicatedOpen += 1;
        } else {
          flexibleOpen += 1;
        }
  ...
  if (dedicatedOpen > 0) {
    const alreadyRostered = teamPicks.some(
      (pick) => getPlayer(state, pick.playerId)?.position === position,
    );
    return Math.min(
      100,
      88 + Math.min(8, (dedicatedOpen - 1) * 4) + (alreadyRostered ? 0 : 4),
    );
  }
  if (flexibleOpen > 0) {
    return 65;
...
function calculateAvailabilityUrgency(
  adp: number | null,
  currentOverallPick: number,
  nextUserPick: number | null,
): number {
  ...
  const window = Math.max(1, nextUserPick - currentOverallPick);
  const margin = adp - nextUserPick;
  return clamp(50 - (margin / window) * 40, 0, 100);
}

function calculateReplacementRank(
  position: PlayerPosition,
  teamCount: number,
...
function findNextSelectionForTeam(
  state: DraftState,
  teamId: string,
  currentOverallPick: number,
): number | null {
  ...
  return (
    state.order.find((slot) => slot.overallPick >= minimumPick && slot.teamId === teamId)
      ?.overallPick ?? null
  );
}
...
function buildReasons(
  ...
  const reasonByComponent: Record<RecommendationComponent, string | null> = {
    rosterNeed:
      metrics.rosterNeed >= 85
        ? `Fills an open ${player.position} starting need.`
        : metrics.rosterNeed >= 60
          ? `Fits an open flexible starting slot.`
          : null,
    tierUrgency:
      metrics.tierUrgency >= 85
        ? `Only ${context.sameTierRemaining ?? 1} ${player.position} remains in this tier.`
        : metrics.tierUrgency >= 65
          ? `The current ${player.position} tier is thinning.`
          : null,
    valueOverReplacement:
      metrics.valueOverReplacement >= 70 && context.projectedPointsAboveReplacement !== null
        ? `Projects ${context.projectedPointsAboveReplacement} points above the ${player.position} replacement level.`
        : null,
    adpValue:
      metrics.adpValue >= 65 && player.adp !== null
        ? `Available ${round(player.adp - context.currentOverallPick)} picks later than market ADP.`
        : metrics.adpValue <= 30 && player.adp !== null
          ? `Costs more than current market ADP, which lowers the grade.`
          : null,
    expectedAvailability:
      metrics.expectedAvailability >= 75 && context.nextUserPick !== null
        ? `Unlikely to remain available through pick ${context.nextUserPick}.`
        : null,
    baseValue:
      metrics.baseValue >= 75
        ? `One of the strongest projected players still available.`
        : null,
    upside:
      metrics.upside >= 70
        ? `Carries above-average upside versus the remaining pool.`
        : null,
    riskSafety:
      metrics.riskSafety >= 70 ? `Profiles as safer than most remaining options.` : null,
  };

  const ordered = RECOMMENDATION_COMPONENTS
    .map((component) => ({
      component,
      priority: metrics[component] * weights[component],
      reason: reasonByComponent[component],
    }))
    .filter(
      (item): item is { component: RecommendationComponent; priority: number; reason: string } =>
        item.reason !== null,
    )
  ...
  if (ordered.length === 0) {
    ordered.push(`Offers balanced value across projection, roster fit, and draft cost.`);
  }
  return ordered.slice(0, 3);
}

function weightedScore(metrics: RecommendationMetrics, weights: RecommendationWeights): number {
  return RECOMMENDATION_COMPONENTS.reduce(
    (sum, component) => sum + metrics[component] * weights[component],
    0,
  );
}
...
function normalizeWeights(weights: RecommendationWeights): RecommendationWeights {
  for (const component of RECOMMENDATION_COMPONENTS) {
    ...
    if (!Number.isFinite(value) || value < 0) {
      throw new RangeError(`Recommendation weight ${component} must be a non-negative number.`);
    }
  }
  const total = RECOMMENDATION_COMPONENTS.reduce(
  ...
  return Object.fromEntries(
    RECOMMENDATION_COMPONENTS.map((component) => [component, weights[component] / total]),
...
function normalizeSigned(value: number | null, values: number[]): number {
  ...
  if (value >= 0) {
    return positiveMaximum === 0 ? 50 : 50 + (value / positiveMaximum) * 50;
  }
  return negativeMinimum === 0
    ? 50
    : 50 - (Math.abs(value) / Math.abs(negativeMinimum)) * 50;
}
...
function getAvailablePlayers(state: DraftState): PlayerDataRecord[] {
  ...
  return state.playerDataRelease.players.filter((player) =>
    available.has(player.canonical_player_id),
  );
}

function getPlayer(state: DraftState, playerId: string): PlayerDataRecord | null {
  return (
    state.playerDataRelease.players.find((player) => player.canonical_player_id === playerId) ??
    null
  );
}
...
function assertDraftAvailable(state: DraftState): void {
  if (state.availablePlayerIds.length === 0) {
    throw new RangeError("The draft has no available players to recommend.");
  }
}
...
function roundMetrics(metrics: RecommendationMetrics): RecommendationMetrics {
  return Object.fromEntries(
    RECOMMENDATION_COMPONENTS.map((component) => [component, round(metrics[component])]),
  ) as unknown as RecommendationMetrics;
}
