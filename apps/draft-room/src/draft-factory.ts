...
export function setRosterCount(
  setup: DraftSetup,
  slot: RosterSlotType,
  count: number,
): DraftSetup {
  ...
  return {
    ...setup,
    rosterCounts,
    rounds: getRosterCapacity(rosterCounts),
  };
}
...
function validateDraftSetup(setup: DraftSetup): void {
  ...
  const rounds = getRosterCapacity(setup.rosterCounts);
  if (rounds < 8 || rounds > 24) {
    throw new RangeError("Custom rosters must contain between 8 and 24 total slots.");
  }
  if (getStarterCapacity(setup.rosterCounts) < 1) {
    throw new RangeError("At least one starting roster slot is required.");
  }
  if (setup.rounds !== rounds) {
    throw new RangeError("Draft rounds must match the configured roster capacity.");
  }
