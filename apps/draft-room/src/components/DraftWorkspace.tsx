...
export function DraftWorkspace({ state, notice, onDraftPlayer, onUndo, onExport, onExit, onCorrectPick, onImportDraft }: DraftWorkspaceProps) {
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return state.playerDataRelease.players
      ...
      .filter((player) => normalizedQuery.length === 0 || [player.display_name, player.position, player.nfl_team ?? ""].join(" ").toLowerCase().includes(normalizedQuery))
      .sort(compareAvailablePlayers)
      .slice(0, 100);
  }, [positionFilter, searchQuery, state.availablePlayerIds, state.playerDataRelease.players]);

  useEffect(() => {
    if (!filteredPlayers.some((player) => player.canonical_player_id === selectedPlayerId)) setSelectedPlayerId(filteredPlayers[0]?.canonical_player_id ?? null);
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent): void => {
      ...
      if ((event.ctrlKey || event.metaKey) && key === "z") { event.preventDefault(); onUndo(); return; }
      if ((event.ctrlKey || event.metaKey) && key === "s") { event.preventDefault(); onExport(); return; }
      if (correctionPickNumber !== null) return;
      if (event.key === "/" && !editableTarget) { event.preventDefault(); searchInputRef.current?.focus(); return; }
      if (key === "c" && !editableTarget && state.picks.length > 0) { event.preventDefault(); setCorrectionPickNumber(state.picks[state.picks.length - 1]!.overallPick); return; }
      ...
      if ((event.key === "ArrowDown" || event.key === "ArrowUp") && canNavigate) {
        ...
        const currentIndex = Math.max(0, filteredPlayers.findIndex((player) => player.canonical_player_id === selectedPlayerId));
        const direction = event.key === "ArrowDown" ? 1 : -1;
        const nextIndex = (currentIndex + direction + filteredPlayers.length) % filteredPlayers.length;
        setSelectedPlayerId(filteredPlayers[nextIndex]!.canonical_player_id);
        return;
      }
      if (event.key === "Enter" && selectedPlayerId !== null && currentSlot !== null && canNavigate) { event.preventDefault(); onDraftPlayer(selectedPlayerId); }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [correctionPickNumber, currentSlot, filteredPlayers, onDraftPlayer, onExport, onUndo, selectedPlayerId, state.picks]);
  ...
  const correctionPick = correctionPickNumber === null ? null : state.picks[correctionPickNumber - 1] ?? null;
  ...
  return (
    <main className="draft-room-shell">
      <header className="draft-header">
        ...
        <div className="header-actions"><button className="secondary-button" type="button" onClick={onExport}>Export</button><button className="secondary-button" type="button" onClick={() => importInputRef.current?.click()}>Import</button><input ref={importInputRef} className="sr-only" type="file" accept="application/json,.json" onChange={(event) => void handleImport(event)} /><button className="secondary-button" type="button" onClick={onUndo} disabled={state.picks.length === 0}>Undo</button><button className="ghost-button" type="button" onClick={onExit}>Exit</button></div>
      </header>

      <section className={`on-clock-banner ${isUserOnClock ? "on-clock-user" : ""}`}>
        {currentSlot === null || currentTeam === null ? <div><p className="eyebrow">Draft complete</p><h2>All {totalPicks} selections are locked in.</h2></div> : <><div className="pick-orb"><span>Pick</span><strong>{currentSlot.overallPick}</strong></div><div className="on-clock-copy"><p className="eyebrow">Round {currentSlot.round} · Pick {currentSlot.pickInRound}</p><h2>{isUserOnClock ? "You are on the clock" : `${currentTeam.name} is on the clock`}</h2><p>{isUserOnClock ? "Choose the position you want to attack, then take the best available player in that group." : "Record the pick from the live draft using the available-player board."}</p></div><div className="clock-team-chip"><span>Draft slot</span><strong>#{currentTeam.draftSlot}</strong></div></>}
      </section>

      <div className="shortcut-strip" aria-label="Keyboard shortcuts"><span><kbd>/</kbd> Search</span><span><kbd>↑</kbd><kbd>↓</kbd> Navigate</span><span><kbd>Enter</kbd> Draft</span><span><kbd>Ctrl/⌘ Z</kbd> Undo</span><span><kbd>C</kbd> Correct latest</span><span className="autosave-status">Autosave active · revision {state.revision}</span></div>
      ...
      <section className="draft-workspace">
        <section className="panel player-board-panel" aria-labelledby="available-title">
          ...
          <div className="player-list" aria-live="polite">{filteredPlayers.length === 0 ? <div className="empty-state"><strong>No players found.</strong><span>Clear the search or select another position.</span></div> : filteredPlayers.map((player) => <PlayerRow key={player.canonical_player_id} player={player} selected={selectedPlayerId === player.canonical_player_id} disabled={currentSlot === null} actionLabel={currentTeam === null ? "Complete" : `Draft for ${currentTeam.name}`} onSelect={() => setSelectedPlayerId(player.canonical_player_id)} onDraft={() => onDraftPlayer(player.canonical_player_id)} />)}</div>
        </section>
        ...
        <section className="panel roster-panel" aria-labelledby="roster-title">
          <div className="panel-heading"><div><p className="eyebrow">League rosters</p><h2 id="roster-title">Team tracker</h2></div><span className="count-badge">{selectedRoster.length} drafted</span></div>
          <div className="team-selector" role="tablist" aria-label="Fantasy teams">{state.teams.map((team) => <button key={team.teamId} type="button" role="tab" aria-selected={selectedTeam.teamId === team.teamId} className={selectedTeam.teamId === team.teamId ? "team-tab-active" : ""} onClick={() => setSelectedTeamId(team.teamId)}><span>{team.isUser ? "YOU" : `#${team.draftSlot}`}</span><strong>{team.name}</strong></button>)}</div>
          <div className="selected-team-summary"><div><p className="eyebrow">Draft slot {selectedTeam.draftSlot}</p><h3>{selectedTeam.name}</h3></div><div className="position-counts">{(["QB", "RB", "WR", "TE"] as PlayerPosition[]).map((position) => <span key={position}>{position} {countPosition(selectedRoster, position, playersById)}</span>)}</div></div>
          <div className="roster-list"><RosterLineup rules={state.settings.rosterSlots} picks={selectedRoster} playersById={playersById} /></div>
        </section>
      </section>

      <section className="panel recent-picks-panel" aria-labelledby="recent-picks-title">
        <div className="panel-heading compact-heading"><div><p className="eyebrow">Draft feed</p><h2 id="recent-picks-title">Recent selections</h2></div><span className="count-badge">Newest first</span></div>
        <div className="recent-picks-grid">{recentPicks.length === 0 ? <div className="empty-state recent-empty"><strong>The board is ready.</strong><span>The first selection will appear here.</span></div> : recentPicks.map((pick) => { const team = state.teams.find((candidate) => candidate.teamId === pick.teamId); const player = playersById.get(pick.playerId); return <article className="recent-pick-card" key={`${pick.overallPick}-${pick.playerId}`}><span className="recent-pick-number">#{pick.overallPick}</span><PositionPill position={player?.position ?? "RB"} /><strong>{player?.display_name ?? pick.playerId}</strong><span>{team?.name ?? pick.teamId}</span><button type="button" className="correct-pick-button" onClick={() => setCorrectionPickNumber(pick.overallPick)} aria-label={`Correct pick ${pick.overallPick}`}>Correct</button></article>; })}</div>
      </section>
      {correctionPick === null ? null : <CorrectionDialog state={state} pick={correctionPick} onClose={() => setCorrectionPickNumber(null)} onCorrect={onCorrectPick} />}
    </main>
  );
}

interface PlayerRowProps { player: PlayerDataRecord; actionLabel: string; disabled: boolean; selected: boolean; onSelect: () => void; onDraft: () => void; }
function PlayerRow({ player, actionLabel, disabled, selected, onSelect, onDraft }: PlayerRowProps) {
  return <article className={`player-row ${selected ? "player-row-selected" : ""}`} data-player-id={player.canonical_player_id} onClick={onSelect}><span className="player-rank">{player.overall_rank ?? "—"}</span><PositionPill position={player.position} /><div className="player-identity"><strong>{player.display_name}</strong><span>{player.nfl_team ?? "FA"} · Bye {player.bye_week ?? "—"} · Tier {player.tier ?? "—"}</span></div><div className="player-market"><span>ADP</span><strong>{player.adp?.toFixed(1) ?? "—"}</strong></div><div className="player-projection"><span>Proj</span><strong>{player.projected_points?.toFixed(1) ?? "—"}</strong></div><button type="button" onClick={(event) => { event.stopPropagation(); onDraft(); }} disabled={disabled} aria-label={`${actionLabel}: ${player.display_name}`}>Draft</button></article>;
}
...
function isEditableTarget(target: HTMLElement | null): boolean { if (target === null) return false; return target.isContentEditable || target.tagName === "INPUT" || target.tagName === "TEXTAREA" || target.tagName === "SELECT"; }
